...
class SceneRenderer:
    ...
    def presentScene(
        ...
        if completionHandler is not None:
            ...
            block = None
        if withTransition is None:
            withTransition = transition
        if withTransition is not None:
            withTransition = withTransition.ID
        if incomingPointOfView is not None:
            incomingPointOfView = incomingPointOfView.ID
        self.ID.presentScene_withTransition_incomingPointOfView_completionHandler_(
            scene.ID, withTransition, incomingPointOfView, block
        )
    ...
    autoenablesDefaultLighting = property(
        getAutoenablesDefaultLighting, setAutoenablesDefaultLighting
    )
    ...
    def prepareObject(self, anObject, shouldAbortBlock=None, block=None):
        if shouldAbortBlock is None:
            shouldAbortBlock = block
        if shouldAbortBlock is not None:
            blockInstance = PrepareObjectShouldAbortBlockBlock(shouldAbortBlock)
            self.ID.prepareObject_shouldAbortBlock_(
                anObject.ID, blockInstance.blockCode
            )
        else:
            self.ID.prepareObject_shouldAbortBlock_(anObject.ID, None)

    def prepareObjects(
        self, objects=None, completionHandler=None, withCompletionHandler=None
    ):
        if completionHandler is None:
    ...
    def hitTest(self, aPoint, options):
        hitList = self.ID.hitTest_options_(
            CGPoint(aPoint[0], aPoint[1]), hitTestOptions(options)
        )
    ...
    def getDelegate(self):
        aSceneRendererDelegate = sceneKit.SceneRendererDelegate.outof(
            self.ID.delegate()
        )
...
class PrepareObjectShouldAbortBlockBlock:
    def __init__(self, block):
        self.blockCode = ObjCBlock(
            self.blockInterface, restype=c_bool, argtypes=[c_void_p]
        )
        self.pCode = block
...
class PrepareObjectsWithCompletionHandlerBlock:
    def __init__(self, block):
        self.blockCode = ObjCBlock(
            self.blockInterface, restype=None, argtypes=[c_void_p, c_bool]
        )
        self.pCode = block
    ...
    def __init__(
        ...
        if crossFadeWithDuration is not None:
            ...
            self.ID = SKTransition.fadeWithColor_duration_(
                ObjCClass("UIColor").color(red=r, green=g, blue=b, alpha=a), duration
            )
...
class SceneRendererDelegate(CInst):
    ...
    _cMethodList = [
        renderer_updateAtTime_,
        renderer_didApplyAnimationsAtTime_,
        renderer_didSimulatePhysicsAtTime_,
        renderer_willRenderScene_atTime_,
        renderer_didRenderScene_atTime_,
        renderer_didApplyConstraintsAtTime_,
    ]
        ...
        if ID is not None:
            ...
            self.methods = [
                ...
                for _a in [
                    ...
                    for anAction in SceneRendererDelegate._actions
                ]
            ]
