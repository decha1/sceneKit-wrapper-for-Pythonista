...
class Geometry(Animatable, Actionable, BoundingVolume, CInst):
    def __init__(self, geometryWithSources=None, elements=None, mdlMesh=None, ID=None):
        if geometryWithSources is not None:
            ...
            self.ID = SCNGeometry.geometryWithSources_elements_(
    ...
    def getGeometryElements(self):
        ...
        if elements is None:
            return None
        return tuple(
            [sceneKit.GeometryElement.outof(anElement) for anElement in elements]
        )
    ...
    def geometrySourcesForSemantic(self, semantic=None):
        return tuple(
            [
                scenekit.GeometrySource.outof(aSource)
                for aSource in self.ID.geometrySourcesForSemantic_(semantic)
            ]
        )
    ...
    def getLevelsOfDetail(self):
        return tuple(
            [
                sceneKit.LevelOfDetail.outof(aLevel)
                for aLevel in self.ID.levelsOfDetail()
            ]
        )
    ...
    wantsAdaptiveSubdivision = property(
        getWantsAdaptiveSubdivision, setWantsAdaptiveSubdivision
    )
...
class LevelOfDetail(CInst):
    def __init__(self, geometry=None, screenSpaceRadius=None, ID=None):
        if screenSpaceRadius is not None:
            self.ID = SCNLevelOfDetail.levelOfDetailWithGeometry_screenSpaceRadius_(
                geometry.ID, screenSpaceRadius
            )
        elif worldSpaceDistance is not None:
            self.ID = SCNLevelOfDetail.levelOfDetailWithGeometry_worldSpaceDistance_(
                geometry.ID, worldSpaceDistance
            )
        elif ID is not None:
            self.ID = ID
        else:
    ...
    @classmethod
    def levelOfDetailWithGeometry(
        cls, geometry=None, screenSpaceRadius=None, worldSpaceDistance=None
    ):
        return cls(
            geometry=geometry,
...
class GeometryTessellator(CInst):
    ...
    edgeTessellationFactor = property(
        getEdgeTessellationFactor, setEdgeTessellationFactor
    )
    ...
    insideTessellationFactor = property(
        getInsideTessellationFactor, setInsideTessellationFactor
    )
    ...
    tessellationFactorScale = property(
        getTessellationFactorScale, setTessellationFactorScale
    )
    ...
    tessellationPartitionMode = property(
        getTessellationPartitionMode, setTessellationPartitionMode
    )
...
class Floor(Geometry):
    ...
    reflectionFalloffStart = property(
        getReflectionFalloffStart, setReflectionFalloffStart
    )
    ...
    reflectionResolutionScaleFactor = property(
        getReflectionResolutionScaleFactor, setReflectionResolutionScaleFactor
    )
    ...
    reflectionCategoryBitMask = property(
        getReflectionCategoryBitMask, setReflectionCategoryBitMask
    )
...
class Plane(Geometry, _GeoHeight, _GeoWidth):
    ...
    cornerRadiusSegmentCount = property(
        getCornerRadiusSegmentCount, setCornerRadiusSegmentCount
    )
...
class Box(Geometry, _GeoHeight, _GeoWidth, _GeoLength):
    def __init__(self, width=0.0, height=0.0, length=0.0, chamferRadius=0.0, ID=None):
        if ID is not None:
            ...
            self.ID = SCNBox.boxWithWidth_height_length_chamferRadius_(
    ...
    def boxWithWidth(cls, width=0.0, height=0.0, length=0.0, chamferRadius=0.0):
        return cls(
            width=width, height=height, length=length, chamferRadius=chamferRadius
        )
...
class Cone(Geometry, _GeoHeight, _GeoRadialSegmentCount):
    def __init__(self, topRadius=0.0, bottomRadius=0.0, height=0.0, ID=None):
        if ID is not None:
            ...
            self.ID = SCNCone.coneWithTopRadius_bottomRadius_height_(
...
class Tube(Geometry, _GeoHeight, _GeoRadialSegmentCount):
    def __init__(self, innerRadius=0.0, outerRadius=0.0, height=0.0, ID=None):
        if ID is not None:
            ...
            self.ID = SCNTube.tubeWithInnerRadius_outerRadius_height_(
...
class Text(Geometry):
    ...
    def setContainerFrame(self, aRect):
        try:
            if len(aRect) == 4:
                aRect = CGRect((aRect[0], aRect[1]), (aRect[2], aRect[3]))
            elif len(aRect) == 2:
                aRect = CGRect(aRect[0], aRect[1])
        except TypeError:
            pass
...
class Shape(Geometry):
    def __init__(self, path=None, extrusionDepth=None, shapeWithPath=None, ID=None):
        if shapeWithPath is None:
            shapeWithPath = path
        if extrusionDepth is not None:
            self.ID = SCNShape.shapeWithPath_extrusionDepth_(
                ObjCInstance(shapeWithPath), extrusionDepth
            )
        elif ID is not None:
            self.ID = ID
        else:
...
class Morpher(CInst):
    ...
    def getTargets(self):
        return tuple(
            [sceneKit.Geometry.outof(aTarget) for aTarget in self.ID.targets()]
        )
...
class Skinner(CInst):
    def __init__(
        ...
        baseGeometry=None,
        bones=None,
        boneInverseBindTransforms=None,
        boneWeights=None,
        boneIndices=None,
        ID=None,
    ):
        if ID is not None:
            ...
            if boneInverseBindTransforms is not None:
                ...
                boneInverseBindTransforms = [
                    SCNMatrix4(*m) for m in boneInverseBindTransforms
                ]
                boneInverseBindTransforms = [
                    NSValue.valueWithSCNMatrix4_(
                        s, restype=c_void_p, argtypes=[SCNMatrix4]
                    )
                    for s in boneInverseBindTransforms
                ]
            ...
            self.ID = SCNSkinner.skinnerWithBaseGeometry_bones_boneInverseBindTransforms_boneWeights_boneIndices_(
                baseGeometry,
                bones,
                boneInverseBindTransforms,
                boneWeights,
                boneIndices,
                restype=c_void_p,
                argtypes=[c_void_p, c_void_p, c_void_p, c_void_p, c_void_p],
            )

    @classmethod
    def skinnerWithBaseGeometry(
        cls,
        baseGeometry=None,
        bones=None,
        boneInverseBindTransforms=None,
    ...
    def getBaseGeometryBindTransform(self):
        ...
        return matrix4Make(
            getCStructValuesAsList(self.ID.baseGeometryBindTransform())
        )  # ??? to be tested if a Skinner can finally be programmed...

    baseGeometryBindTransform = property(
        getBaseGeometryBindTransform, setBaseGeometryBindTransform
    )
