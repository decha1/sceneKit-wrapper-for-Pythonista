...
(
    GeometrySourceSemanticVertex,
    GeometrySourceSemanticNormal,
    GeometrySourceSemanticTexcoord,
    GeometrySourceSemanticColor,
    GeometrySourceSemanticTangent,
    GeometrySourceSemanticEdgeCrease,
    GeometrySourceSemanticVertexCrease,
    GeometrySourceSemanticBoneIndices,
    GeometrySourceSemanticBoneWeights,
) = _geometrySourceSemantics
...
class GeometrySource(CInst):
    def __init__(
        ...
        semantic=None,
        vectorCount=None,
        floatComponents=None,
        componentsPerVector=None,
        bytesPerComponent=None,
        dataOffset=None,
        dataStride=None,
        geometrySourceWithVertices=None,
        geometrySourceWithNormals=None,
        geometrySourceWithTextureCoordinates=None,
        geometrySourceWithData=None,
        geometrySourceWithCData=None,
        ID=None,
    ):
        if geometrySourceWithVertices is not None:
            ...
            self.ID = SCNGeometrySource.geometrySourceWithVertices_count_(
                byref(verts_array),
                len(verts),
                restype=c_void_p,
                argtypes=[POINTER(SCNVector3ArrayN), c_long],
            )
            ...
            self.ID = SCNGeometrySource.geometrySourceWithNormals_count_(
                byref(norms_array),
                len(norms),
                restype=c_void_p,
                argtypes=[POINTER(SCNVector3ArrayN), c_long],
            ...
            coords = [
                CGPoint(*aCoord) for aCoord in geometrySourceWithTextureCoordinates
            ]
            coords_array = CGPointArrayN(*coords)
            self.ID = SCNGeometrySource.geometrySourceWithTextureCoordinates_count_(
                byref(coords_array),
                len(coords),
                restype=c_void_p,
                argtypes=[POINTER(CGPointArrayN), c_long],
            )
        elif geometrySourceWithData is not None:
            dataC = ns(bytes(geometrySourceWithData))
            self.ID = SCNGeometrySource.geometrySourceWithData_semantic_vectorCount_floatComponents_componentsPerVector_bytesPerComponent_dataOffset_dataStride_(
                dataC,
                semantic,
                vectorCount,
                floatComponents,
    ...
    def interleavedGeometrySourceWithVectors(cls, sourceList, semanticList):
        ...
        for i in range(len(sourceList)):
            ...
            sourceParams.append(
                (floatComponents, componentsPerVector, bytesPerComponent, dataOffset)
            )
        ...
        data = NSData.dataWithBytes_length_(
            byref(vertexStructArray), sizeof(vertexStructArray)
        )
        for i in range(len(sourceList)):
            returnSources.append(
                cls.outof(
                    SCNGeometrySource.geometrySourceWithData_semantic_vectorCount_floatComponents_componentsPerVector_bytesPerComponent_dataOffset_dataStride_(
                        data,
                        semanticList[i],
                        vectorCount,
                        sourceParams[i][0],
        ...
        for i in range(self.vectorCount):
            ...
            for j in range(self.componentsPerVector):
                if self.floatComponents:
                    dataC = create_string_buffer(
                        data[
                            (i * self.dataStride + self.dataOffset)
                            + j
                            * self.bytesPerComponent : (
                                i * self.dataStride + self.dataOffset
                            )
                            + j * self.bytesPerComponent
                            + self.bytesPerComponent
                        ],
                        size=self.bytesPerComponent,
                    )
...
class GeometryElement(CInst):
    def __init__(
        self, geometryElementWithData=None, primitiveType=None, mdlSubMesh=None, ID=None
    ):
        if geometryElementWithData is not None:
            ...
            for i in range(len(geometryElementWithData)):
                indexes_array[
                    i * bytesPerIndex : (i + 1) * bytesPerIndex
                ] = geometryElementWithData[i].to_bytes(bytesPerIndex, sys.byteorder)
            datIndexes = ns(bytes(indexes_array))
            self.ID = SCNGeometryElement.geometryElementWithData_primitiveType_primitiveCount_bytesPerIndex_(
                datIndexes, primitiveType.value, count, bytesPerIndex
            )
        elif mdlSubMesh is not None:
            self.ID = SCNGeometryElement.geometryElementWithMDLSubmesh_(mdlSubMesh)
            if self.ID is None:
                raise RuntimeError(
                    "geometryElementWithMDLSubmesh failed. Wrong MDL asset?"
                )
        elif ID is not None:
            self.ID = ID
        else:
    ...
    @classmethod
    def geometryElementWithData(
        cls, data=None, primitiveType=None, primitiveCount=None, bytesPerIndex=None
    ):
    ...
    def getData(self):
        ...
        for i in range(0, len(data) // bytesPerIndex):
            dataRet.append(
                int.from_bytes(
                    data[i * bytesPerIndex : (i + 1) * bytesPerIndex], sys.byteorder
                )
            )
        return tuple(dataRet)
    ...
    minimumPointScreenSpaceRadius = property(
        getMinimumPointScreenSpaceRadius, setMinimumPointScreenSpaceRadius
    )
    ...
    maximumPointScreenSpaceRadius = property(
        getMaximumPointScreenSpaceRadius, setMaximumPointScreenSpaceRadius
    )
