...
class SpeedNeedle:
    def __init__(self):
        ...
        self.image_base = self.image_base.resize(
            (int(size[0] * 0.55), int(size[1] * 0.55))
        )
        size = self.image_base.size
    ...
    def get_zRotation(self):
        return self._zRotation / 180.0 * math.pi

    def set_zRotation(self, angle):
        self._zRotation = angle / math.pi * 180.0
        self.draw_needle()
...
class OverlayScene:
    def __init__(self, main_view):
        ...
        self.myImage_view.frame = (
            20,
            int(main_view.frame[3] - 1.5 * myImage.size.height),
            myImage.size.width,
            myImage.size.height,
        )
        ...
        self.mySpeedNeedle.view.frame = (
            self.myImage_view.frame[0],
            self.myImage_view.frame[1] + self.myImage_view.frame[3] // 2 - 8,
            self.myImage_view.frame[2],
            self.myImage_view.frame[3],
        )
        ...
        self.myCameraButton.view.frame = (
            main_view.frame[2] - 20 - self.myCameraButton.image.size.width,
            int(main_view.frame[3] - 1.5 * myImage.size.height + 10),
            self.myCameraButton.image.size.width,
            self.myCameraButton.image.size.height,
        )
