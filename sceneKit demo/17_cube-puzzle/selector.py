...
class SelectorStage:
    def __init__(self):
        ...
        self.selector_views[0].scene.background.addAnimationPlayer(
            self.animation_player, "highlight"
        )
    ...
    def next_selector_animation(self, animation, receiver, completed):
        if completed:
            ...
            ].scene.background.removeAllAnimations()
            self.animation_index += 1
            if self.animation_index == len(self.selector_views):
                self.animation_index = 0
            self.selector_views[
                self.animation_index
            ].scene.background.addAnimationPlayer(self.animation_player, "highlight")
    ...
    def tap(self, location):
        for index, aView in enumerate(self.selector_views):
            aFrame = aView.frame
            if (
                aFrame[0] < location.x < aFrame[0] + aFrame[2]
                and aFrame[1] < location.y < aFrame[1] + aFrame[3]
            ):
                data.hud_layer.back_button.hidden = False
                data.hud_layer.restart_button.hidden = False
                main.MainViewController.next_stage(
                    manual.ManualStage(Puzzle_variant(index))
                )
    ...
    def layout(self):
        ...
        for i in range(len(self.selector_views)):
            ...
            column = i % columns
            if row != rows - 1:
                self.selector_views[i].frame = (
                    int(0.4 * width + 1.3 * column * width),
                    int(0.4 * height + 1.3 * row * height),
                    width,
                    height,
                )
            else:
                self.selector_views[i].frame = (
                    int(0.4 * width + missing / 2 * 1.3 * width + 1.3 * column * width),
                    int(0.4 * height + 1.3 * row * height),
                    width,
                    height,
                )
            camera_node = self.selector_views[i].scene.rootNode.childNodeWithName(
                "selector_camera"
            )
            camera_node.position = (
                (0.0, 7.0, 11.0) if width / height > 0.8 else (0.0, 9.5, 14.5)
            )
