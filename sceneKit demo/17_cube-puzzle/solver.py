...
class Solver:
    def __init__(self, puzzle_variant):
        self.puzzle_cubes = sum(
            len(nodes) for nodes in data.reference_pieces[puzzle_variant.value].values()
        )
        ...
        try:
            ...
            if Vector3 is None:
                raise ImportError(
                    'Module Pyrr is needed to recreate the shapes.P file. Ether re-download the shapes.P file or run "pip install pyrr".'
                )
            self.shapes_dict = {}
            for name, nodes in data.reference_pieces[puzzle_variant.value].items():
        ...
        self.pieces = [
            Piece(name, self.shapes_dict[name])
            for name in data.reference_pieces[puzzle_variant.value].keys()
        ]
        ...
        try:
            with open("resources/" + puzzle_variant.name + "-solution.P", "rb") as fs:
                ...
                if self.empty_solutions:
                    if len(
                        data.reference_pieces[puzzle_variant.value][
                            self.empty_solutions[0][0][0][0]
                        ]
                    ) != len(self.empty_solutions[0][0][0][1]):
                        raise pickle.UnpicklingError
        except (FileNotFoundError, pickle.UnpicklingError, KeyError):
    ...
    def solve_it(self, input):
        ...
        for aPiece_name in input:
            ...
            aPiece.puzzle_shape = frozenset(
                [
                    Cube(int(round(aNode.x)), int(round(aNode.y)), int(round(aNode.z)))
                    for aNode in shape
                ]
            )
            aPiece.shapes = {
                aPiece.puzzle_shape: self.shapes_dict[aPiece.name][aPiece.puzzle_shape]
            }
            ...
            if len(cubes_used) != self.puzzle_cubes:
                ...
                while solver_pieces:
                    ...
                    for aPiece in solver_pieces:
                        ...
                        remainder_cubes = set()
                        for aRemainder_piece in (
                            _ for _ in solver_pieces if _ != aPiece
                        ):
    ...
    def generate_shapes(self, nodes):
        ...
        for x_rot in (3, 2, 1, 0):
            ...
            rotated_nodes_x = self.shape_constrained(
                [new_rotation * aNode for aNode in nodes], zero_align=True
            )
            for y_rot in (3, 2, 1, 0):
                new_rotation = quaternions[(0, y_rot, 0)]
                rotated_nodes_y = self.shape_constrained(
                    [new_rotation * aNode for aNode in rotated_nodes_x], zero_align=True
                )
                for z_rot in (3, 2, 1, 0):
                    new_rotation = quaternions[(0, 0, z_rot)]
                    rotated_nodes = self.shape_constrained(
                        [new_rotation * aNode for aNode in rotated_nodes_y],
                        zero_align=True,
                    )
                    for x_shift in (2, 1, 0):
                        for y_shift in (2, 1, 0):
                            for z_shift in (-2, -1, 0):
                                ...
                                shifted_nodes = [
                                    new_shift + aNode for aNode in rotated_nodes
                                ]
    ...
    def shape_constrained(self, shape_nodes, zero_align=False):
        min_box = Vector3([100.0, 100.0, 100.0])
        max_box = Vector3([-100.0, -100.0, -100.0])
        ...
        if not zero_align:
            if min_box.x < 0:
                ...
                offset.x = float(max_box.x - 2.0)
            if min_box.y < -6:
                offset.y = float(min_box.y + 6.0)
            elif max_box.y > -4:
                offset.y = float(max_box.y + 4.0)
            if min_box.z < -2:
                offset.z = float(min_box.z + 2.0)
            elif max_box.z > 0:
