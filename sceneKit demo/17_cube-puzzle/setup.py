...
def setup_elementary_cube_geometry():
    geometry = scn.Box(1.0, 1.0, 1.0, 0.0)
    ...
    for name, cubes in data.reference_pieces[puzzle_variant.value].items():
        ...
        for aNode in nodes:
            aNode.geometry.firstMaterial.diffuse.intensity = (
                0.7 - len(pieces) / len(data.reference_pieces[puzzle_variant.value])
            ) + 0.3
...
rotate_action = scn.Action.repeatActionForever(
    scn.Action.rotateBy(0, math.pi * 2, 0, 25.0)
)
...
def setup_selector_views():
    ...
    for aVariant in Puzzle_variant:
        ...
        title_geometry.font = ("Chalkboard SE", 4.0)
        title_geometry.firstMaterial.contents = (0.48, 0.14, 0.14)
        ...
        for index, aPiece in enumerate(piece_set.pieces):
            ...
            show_piece.rotation = (
                ...
                1,
                0,
                index / pieces * 2 * math.pi - math.pi / 4.0,
            )
            show_piece.position = scn.Vector3(
                4.0 * math.sin(index / pieces * 2 * math.pi),
                0.0,
                4.0 * math.cos(index / pieces * 2 * math.pi),
            )
        ...
        camera_node.position = (0.0, 7.0, 11.0)
        camera_node.lookAt((0.0, 0.0, 0.0))
...
def setup_center_view(piece_set):
    ...
    camera_node.position = (8.0, 10.0, 12)
    camera_node.lookAt((1.0, 3.0, -1.5))
    camera_node.camera = scn.Camera()
...
def setup_drawer_views(piece_set):
    for aPiece in piece_set.pieces:
        ...
        camera_node = scn.Node()
        camera_node.position = (2.5, 3.0, 2.5)
        camera_node.lookAt(((min.x + max.x) / 2 + 1.0, (min.y + max.y) / 2 + 0.5, 0.0))
        camera_node.camera = scn.Camera()
...
def setup_show_drawer_animation():
    ...
    core_animation.toValue = (0.4, 1.0, 0.0, 0.9)
...
def setup_axes():
    ...
    vertical_axis_rode = scn.Cylinder(0.015, 11.0)
    ...
    x_axis_rotate_control_node_1.position = (0.0, -axis_rode.height / 2 + 1, 0.0)
    ...
    x_axis_rotate_control_node_2.position = (0.0, axis_rode.height / 2 - 1, 0.0)
    ...
    x_axis_shift_control_node_1.position = (0.0, -axis_rode.height / 2, 0.0)
    x_axis_shift_control_node_1.rotation = (0.0, 0.0, 1.0, -math.pi)
    ...
    x_axis_shift_control_node_2.position = (0.0, axis_rode.height / 2, 0.0)
    ...
    y_axis_rotate_control_node_1.position = (0.0, -vertical_axis_rode.height / 2, 0.0)
    ...
    y_axis_rotate_control_node_2.position = (0.0, vertical_axis_rode.height / 2, 0.0)
    ...
    z_axis_rotate_control_node_1.position = (0.0, -axis_rode.height / 2 + 1, 0.0)
    ...
    z_axis_rotate_control_node_2.position = (0.0, axis_rode.height / 2 - 1, 0.0)
    ...
    z_axis_shift_control_node_1.position = (0.0, -axis_rode.height / 2, 0.0)
    z_axis_shift_control_node_1.rotation = (0.0, 0.0, 1.0, math.pi)
    ...
    z_axis_shift_control_node_2.position = (0.0, axis_rode.height / 2, 0.0)
