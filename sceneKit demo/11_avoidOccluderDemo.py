...
class Demo:
    ...
    def main(self):
        ...
        ball_geometry.firstMaterial.diffuse.contents = (0.48, 0.48, 0.48)
        ball_geometry.firstMaterial.specular.contents = (0.88, 0.88, 0.88)
        ...
        occluder_geometry = scn.Box(0.3, 2.0, 15.0, 0.2)
        occluder_geometry.firstMaterial.diffuse.contents = (0.91, 0.91, 0.91)
        occluder_node = scn.Node.nodeWithGeometry(occluder_geometry)
        occluder_node.position = (0.0, 0.8, 0.0)
        root_node.addChildNode(occluder_node)

        self.orbit_r = 10
        self.omega_speed_1 = math.pi / 1500
        self.omega_speed_2 = 1.5 * self.omega_speed_1
        self.ball_node_1.position = (self.orbit_r, 0.5, 0.0)
        self.ball_node_2.position = (0.0, 0.5, self.orbit_r)
    ...
    def update(self, view, atTime):
        ...
        self.omega_1 = -math.atan2(
            det2((pos_1.x, pos_1.z), (1.0, 0.0)), dot((pos_1.x, pos_1.z), (1.0, 0.0))
        )
        self.omega_2 = -math.atan2(
            det2((pos_2.x, pos_2.z), (1.0, 0.0)), dot((pos_2.x, pos_2.z), (1.0, 0.0))
        )
        self.omega_1 += self.omega_speed_1
        self.omega_2 += self.omega_speed_2
        self.ball_node_1.position = (
            self.orbit_r * math.cos(self.omega_1),
            0.5,
            self.orbit_r * math.sin(self.omega_1),
        )
        self.ball_node_2.position = (
            self.orbit_r * math.cos(self.omega_2),
            0.5,
            self.orbit_r * math.sin(self.omega_2),
        )
