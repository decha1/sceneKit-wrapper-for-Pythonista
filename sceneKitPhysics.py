...
(
    PhysicsShapeOptionCollisionMargin,
    PhysicsShapeKeepAsCompoundKey,
    PhysicsShapeScaleKey,
    PhysicsShapeTypeKey,
    PhysicsShapeTypeBoundingBox,
    PhysicsShapeTypeConcavePolyhedron,
    PhysicsShapeTypeConvexHull,
) = _physicsShapeOptions
...
(
    PhysicsTestBackfaceCullingKey,
    PhysicsTestCollisionBitMaskKey,
    PhysicsTestSearchModeKey,
    PhysicsTestSearchModeAll,
    PhysicsTestSearchModeAny,
    PhysicsTestSearchModeClosest,
) = _physicsTestOptions
...
class PhysicsWorld(CInst_NoCache):
    ...
    def getAllBehaviors(self):
        return tuple(
            [
                sceneKit.PhysicsBehavior.outof(aBehavior)
                for aBehavior in self.ID.allBehaviors()
            ]
        )
    ...
    def getContactDelegate(self):
        aPhysicsContactDelegate = sceneKit.PhysicsContactDelegate.outof(
            self.ID.contactDelegate()
        )
    ...
    def contactTestBetween(self, bodyA=None, bodyB=None, options=None):
        contactList = self.ID.contactTestBetweenBody_andBody_options_(
            bodyA.ID, bodyB.ID, options
        )
    ...
    def rayTestWithSegment(self, origin=None, dest=None, options=None):
        hitList = self.ID.rayTestWithSegmentFromPoint_toPoint_options_(
            vector3Make(origin), vector3Make(dest), options
        )
    ...
    def convexSweepTest(
        self, shape=None, fromTransform=None, toTransform=None, options=None
    ):
        contactList = (
            self.id.convexSweepTestWithShape_fromTransform_toTransform_options_(
                shape.ID, matrix4Make(fromTransform), matrix4Make(toTransform), options
            )
        )
        return tuple([sceneKit.PhysicsContact.outof(aCon) for aCon in contactList])

    def convexSweepTestWithShape(
        self, shape=None, fromTransform=None, toTransform=None, options=None
    ):
        return self.convexSweepTest(
            shape=shape,
...
class PhysicsField(CInst):
    def __init__(
        ...
        dragField=None,
        vortexField=None,
        radialGravityField=None,
        linearGravityField=None,
        noiseField=None,
        turbulenceField=None,
        springField=None,
        electricField=None,
        magneticField=None,
        smoothness=None,
        ...
        if ID is not None:
            ...
            self.ID = SCNPhysicsField.noiseFieldWithSmoothness_animationSpeed_(
                smoothness, speed
            )
        elif turbulenceField is not None:
            self.ID = SCNPhysicsField.turbulenceFieldWithSmoothness_animationSpeed_(
                smoothness, speed
            )
            ...
            self.ID = SCNPhysicsField.customFieldWithEvaluationBlock_(
                self.fieldForceEvaluatorBlock.blockCode
            )
        else:
    ...
    def __init__(self, block):
        try:
            self.blockCode = ObjCBlock(
                self.blockInterface,
                restype=SCNVector3,
                argtypes=[c_void_p, SCNVector3, SCNVector3, c_float, c_float, c_double],
            )
        except TypeError:
            self.blockCode = ObjCBlock(
                self.blockInterfaceLimited,
                restype=c_float,
                argtypes=[c_void_p, SCNVector3, SCNVector3, c_float, c_float, c_double],
            )
        self.pCode = block
...
class PhysicsBody(CInst):
    ...
    usesDefaultMomentOfInertia = property(
        getUsesDefaultMomentOfInertia, setUsesDefaultMomentOfInertia
    )
    ...
    continuousCollisionDetectionThreshold = property(
    ...
    def applyForce(self, direction=None, position=None, impulse=False, atPosition=None):
        ...
        if position is None:
            ...
            self.ID.applyForce_atPosition_impulse_(
                vector3Make(direction), vector3Make(position), impulse
            )

    def applyForceAtPosition(
        self, direction=None, position=None, impulse=False, atPosition=None
    ):
        self.applyForce(
            direction=direction,
            position=position,
    ...
    angularRestingThreshold = property(
        getAngularRestingThreshold, setAngularRestingThreshold
    )
    ...
    linearRestingThreshold = property(
        getLinearRestingThreshold, setLinearRestingThreshold
    )
...
class PhysicsHingeJoint(CInst):
    def __init__(
        ...
        if ID is not None:
            ...
            self.ID = SCNPhysicsHingeJoint.jointWithBody_axis_anchor_(
                bodyA.ID, vector3Make(axisA), vector3Make(anchorA)
            )
        else:
            self.ID = (
                SCNPhysicsHingeJoint.jointWithBodyA_axisA_anchorA_bodyB_axisB_anchorB_(
                    bodyA.ID,
                    vector3Make(axisA),
                    vector3Make(anchorA),
                    bodyB.ID,
    ...
    @classmethod
    def jointWithBodyA(
        cls, bodyA=None, axisA=None, anchorA=None, bodyB=None, axisB=None, anchorB=None
    ):
        return cls(
            bodyA=bodyA,
            axisA=axisA,
...
class PhysicsSliderJoint(PhysicsHingeJoint):
    ...
    motorTargetLinearVelocity = property(
        getMotorTargetLinearVelocity, setMotorTargetLinearVelocity
    )
    ...
    motorTargetAngularVelocity = property(
        getMotorTargetAngularVelocity, setMotorTargetAngularVelocity
    )
...
class PhysicsBallSocketJoint(CInst):
    def __init__(self, bodyA=None, anchorA=None, bodyB=None, anchorB=None, ID=None):
        if ID is not None:
            ...
            self.ID = SCNPhysicsBallSocketJoint.jointWithBody_anchor_(
                bodyA.ID, vector3Make(anchorA)
            )
        else:
            self.ID = SCNPhysicsBallSocketJoint.jointWithBodyA_anchorA_bodyB_anchorB_(
                bodyA.ID, vector3Make(anchorA), bodyB.ID, vector3Make(anchorB)
            )
    ...
    @classmethod
    def joint(
        cls, body=None, anchor=None, bodyA=None, anchorA=None, bodyB=None, anchorB=None
    ):
        if bodyB is None:
            if bodyA is None:
                return cls(bodyA=body, anchorA=anchor)
...
class PhysicsConeTwistJoint(CInst):
    def __init__(self, bodyA=None, frameA=None, bodyB=None, frameB=None, ID=None):
        if ID is not None:
            ...
            self.ID = SCNPhysicsConeTwistJoint.jointWithBody_frame_(
                bodyA.ID, matrix4Make(frameA)
            )
        else:
            self.ID = SCNPhysicsConeTwistJoint.jointWithBodyA_frameA_bodyB_frameB_(
                bodyA.ID, matrix4Make(frameA), bodyB.ID, matrix4Make(frameB)
            )
    ...
    @classmethod
    def joint(
        cls, body=None, frame=None, bodyA=None, frameA=None, bodyB=None, frameB=None
    ):
        if bodyB is None:
            if bodyA is None:
                return cls(bodyA=body, frameA=frame)
...
class PhysicsVehicle(CInst):
    def __init__(self, chassisBody=None, wheels=None, ID=None):
        if ID is not None:
            ...
            kind = desc[4 : desc.index(":")]
            if kind == "PhysicsBody":
                self.ID = SCNPhysicsVehicle.vehicleWithChassisBody_wheels_(
                    chassisBody.ID, wheels
                )
            else:
    ...
    def getWheels(self):
        return tuple(
            [sceneKit.PhysicsVehicleWheel.outof(aWheel) for aWheel in self.ID.wheels()]
        )

    wheels = property(getWheels, None)

    def applyEngineForce(
        self, engineForce=None, forWheelAtIndex=None, value=0.0, index=0
    ):
        if engineForce is None:
            engineForce = value
        if forWheelAtIndex is None:
            forWheelAtIndex = index
        self.ID.applyEngineForce_forWheelAtIndex_(engineForce, forWheelAtIndex)

    def applyBrakingForce(
        self, brakingForce=None, forWheelAtIndex=None, value=0.0, index=0
    ):
        if brakingForce is None:
            brakingForce = value
        if forWheelAtIndex is None:
            forWheelAtIndex = index
    ...
    def setSteeringAngle(
        self, steeringAngle=None, forWheelAtIndex=None, value=0.0, index=0
    ):
...
class PhysicsVehicleWheel(CInst):
    ...
    maximumSuspensionTravel = property(
        getMaximumSuspensionTravel, setMaximumSuspensionTravel
    )
...
class PhysicsShape(CInst):
    def __init__(
        ...
        if ID is not None:
            ...
            if transforms is not None:
                ...
                transforms = [
                    NSValue.valueWithSCNMatrix4_(
                        s, restype=c_void_p, argtypes=[SCNMatrix4]
                    )
                    for s in transforms
                ]
    ...
    def getTransforms(self):
        return tuple(
            [
                matrix4Make(getCStructValuesAsList(aTrans.SCNMatrix4Value()))
                for aTrans in self.ID.transforms()
            ]
        )
...
class PhysicsContactDelegate(CInst):
    ...
    _cMethodList = [
        physicsWorld_didBeginContact_,
        physicsWorld_didUpdateContact_,
        physicsWorld_didEndContact_,
    ]
        ...
        if ID is not None:
            ...
            self.methods = [
                ...
                for _a in [
                    ...
                    for anAction in PhysicsContactDelegate._actions
                ]
            ]
