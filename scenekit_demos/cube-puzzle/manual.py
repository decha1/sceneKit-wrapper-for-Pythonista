...
class ManualStage:
    ...
    def restart(self):
        ...
        scn.Transaction.animationDuration = 1.0
    ...
    def scene_views(self):
        return [self.center_view] + [
            aPiece.drawer_view for aPiece in self.piece_set.pieces
        ]
    ...
    def tap(self, location):
        for aPiece in self.piece_set.pieces:
            ...
            if (
                drawer_frame[0] < location.x < drawer_frame[0] + drawer_frame[2]
                and drawer_frame[1] < location.y < drawer_frame[1] + drawer_frame[3]
            ):
                self.drawer_tapped(aPiece)
                return
        if (
            self.center_view.frame[0]
            < location.x
            < self.center_view.frame[0] + self.center_view.frame[2]
            and self.center_view.frame[1]
            < location.y
            < self.center_view.frame[1] + self.center_view.frame[3]
        ):
    ...
    def center_view_tapped(self, location):
        ...
        hit_list = self.center_view.hitTest(
            (x, y),
            {
                scn.HitTestOptionSearchMode: scn.HitTestSearchMode.Closest,
                scn.HitTestOptionCategoryBitMask: 2,
            },
        )
        if not hit_list:
            return
        ...
        tray_piece = next(
            (
                aCandidate
                for aCandidate in self.piece_set.pieces
                if aCandidate.location == Piece_location.Tray
            ),
            None,
        )
        scn.Transaction.begin()
    ...
    def hint(self):
        ...
        tray_pieces = [
            aPiece
            for aPiece in self.piece_set.pieces
            if aPiece.location == Piece_location.Tray
        ]
        ...
        candidate_pieces = [
            aPiece for aPiece in self.piece_set.pieces if aPiece.name in candidate_names
        ]
    ...
    def show_next_tray_position(self):
        ...
        tail = "".join(
            "⚫️"
            for i in range(
                len(self.tray_piece_transforms) - self.tray_piece_transforms_index - 1
            )
        )
    ...
    def layout(self):
        ...
        for i in range(len(self.piece_set.pieces)):
            ...
            if data.horizontal:
                ...
                y = (
                    h
                    - (odd * side_with_gap if not left_side else 0) // 2
                    - (h - half * side - (half - 1) * gap) // 2
                    - side
                    - tile * side_with_gap
                )
            else:
                x = (
                    (odd * side_with_gap if not top_side else 0) // 2
                    + (w - half * side - (half - 1) * gap) // 2
                    + tile * side_with_gap
                )
        ...
        y = (
            (h - half * side - (half - 1) * gap) // 2 + 10
            if data.horizontal
            else 20 + side // 2 + side + gap
        )

        self.center_view.frame = (
            (x, y, w - 2 * x, h - 2 * y)
            if data.horizontal
            else (x, y, w - 2 * x, h - 2 * y)
        )
        ...
        box_node.position = (1.0, 1.0, -1.0) if data.horizontal else (-0.5, 3, -1)
        box_node.scale = (1.0, 1.0, 1.0) if data.horizontal else (1.3, 1.3, 1.3)

        camera_node = self.center_view.scene.rootNode.childNodeWithName("center_camera")
        camera_node.position = (6.0, 8.5, 9.5) if data.horizontal else (9.2, 10.0, 13.0)
