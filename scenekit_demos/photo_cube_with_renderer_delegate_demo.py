...
cube_images_orig = [
    ui.Image.named(cube_image_names[i]) for i in range(len(cube_image_names))
]
...
with ui.ImageContext(image_width, image_height) as ctx:
    ...
    rrect.line_width = 4
    ui.set_color((0.91, 0.91, 0.91))
    rrect.fill()
    ui.set_color("black")
    rrect.stroke()
    d = 2
    rrect = ui.Path.rounded_rect(
        0 + d, 0 + d, image_width - 2 * d, image_height - 2 * d, 2
    )
    rrect.line_width = 2
    ui.set_color((0.6, 0.6, 0.6))
    rrect.stroke()
...
def main():
    ...
    scene.background.wrapS, scene.background.wrapT = (
        scn.WrapMode.Repeat,
        scn.WrapMode.Repeat,
    )
    ...
    cube_geometry_inside = scn.Box(
        width=1 - 0.001, height=1 - 0.001, length=1 - 0.001, chamferRadius=0.04
    )
    ...
    cube_node.position = (0.0, 0.0, -5.0)
