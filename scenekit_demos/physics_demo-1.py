...
class Demo:
    ...
    def main(self):
        ...
        rope_element_geometry.firstMaterial.diffuse.content = (0.77, 0.67, 0.09)

        rope_nodes = []
        for i in range(rope_seg_nr):
            rope_nodes.append(scn.Node.nodeWithGeometry(rope_element_geometry))
            if i > 0:
                rope_nodes[-1].position = (0.0, -rope_seg_length, 0.0)
        ...
        physics_world.addBehavior(
            scn.PhysicsBallSocketJoint.joint(
                rope_nodes[0].physicsBody, (0, rope_seg_length / 2, 0)
            )
        )
        physics_world.addBehavior(
            scn.PhysicsBallSocketJoint.joint(
                rope_nodes[-1].physicsBody,
                (0, -rope_seg_length / 2, 0),
