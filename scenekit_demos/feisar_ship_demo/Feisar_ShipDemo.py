...
class Demo:
    ...
    def main(self):
        ...
        feisar_material.roughness.contents = (0.79, 0.79, 0.79)
        feisar_material.metalness.contents = (0.11, 0.11, 0.11)
        ...
        blue_emitter.emission.contents = (0.0, 0.35, 1.0)
        white_emitter = scn.Material()
        white_emitter.diffuse.contents = "black"
        white_emitter.emission.contents = (0.86, 0.98, 1.0)
        ...
        nose_node.position = (0.0, 4.35, 161.0)
        nose_node.castsShadow = False
        nose_node.runAction(pulse_action)
        feisar_node.addChildNode(nose_node)

        front_grille_p0 = scn.vector3Make((0.0, 10.0, 137.6))
        front_grille_p1 = scn.vector3Make((0.0, 20.0, 153.5))
        ...
        for i in range(grille_nr):
            ...
            aNode.scale = (
                1.0 + front_grille_ds1.x * i / grille_nr,
                1.0 + front_grille_ds1.y * i / grille_nr,
                1.0 + front_grille_ds1.z * i / grille_nr,
            )
            aNode.rotation = (0.0, 0.0, 1.0, math.pi / 2)
            ...
            aNode.runAction(
                scn.Action.repeatActionForever(
                    scn.Action.sequence([grille_action, grille_action.reversedAction()])
                )
            )
