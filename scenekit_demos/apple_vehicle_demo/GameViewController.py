...
class GameViewController:
    ...
    def setupEnvironment(self, scene):
        ...
        scale[0], scale[5], scale[10] = 2.0, 2.0, 1.0
    ...
    def setupSceneElements(self, scene):
        # add walls
        wall = scn.Node(
            geometry=scn.Box(width=400, height=100, length=4, chamferRadius=0)
        )
        wall.geometry.firstMaterial.diffuse.contents = "resources/wall.jpg"
        scale = list(scn.Matrix4Identity)
        scale[0], scale[5], scale[13], scale[10] = 24.0, 2.0, 1.0, 1.0
        ...
        # add wooden blocks
        self.addWoodenBlockToScene(
            scene,
            imageName="resources/WoodCubeA.jpg",
            position=scn.Vector3(-10, 15, 10),
        )
        self.addWoodenBlockToScene(
            scene, imageName="resources/WoodCubeB.jpg", position=scn.Vector3(-9, 10, 10)
        )
        self.addWoodenBlockToScene(
            scene,
            imageName="resources/WoodCubeC.jpg",
            position=scn.Vector3(20, 15, -11),
        )
        self.addWoodenBlockToScene(
            scene, imageName="resources/WoodCubeA.jpg", position=scn.Vector3(25, 5, -20)
        )
        ...
        for _ in range(4):
            self.addWoodenBlockToScene(
                scene,
                imageName="resources/WoodCubeA.jpg",
                position=scn.Vector3(
                    random.randint(-30, 30), 20, random.randint(-20, 20)
                ),
            )
            self.addWoodenBlockToScene(
                scene,
                imageName="resources/WoodCubeB.jpg",
                position=scn.Vector3(
                    random.randint(-30, 30), 20, random.randint(-20, 20)
                ),
            )
        ...
        block.geometry = scn.Box(width=22, height=2.0, length=34, chamferRadius=0)
        ...
        scale = list(scn.Matrix4Identity)
        scale[0], scale[5], scale[10] = 2.0, 1.0, 1.0
    ...
    def addTrainToScene(self, scene=None, pos=None):
        ...
        self.smoke.idleDurationVariation = 0.0
        ...
        self.smoke.particleLifeSpan = 5.0
        self.smoke.particleLifeSpanVariation = 0.0
        self.smoke.particleVelocity = 6.0
        self.smoke.particleVelocityVariation = 1
        self.smoke.particleImage = ui.Image.named("resources/tex_smoke.png")
        self.smoke.particleSize = 3.0
        self.smoke.particleSizeVariation = 0.5
        self.smoke.particleIntensity = 1.0
        self.smoke.particleIntensityVariation = 0.0
        self.smoke.stretchFactor = 0.0
        self.smoke.particleColor = (0.41, 0.42, 0.48)
        ...
        joint = scn.PhysicsBallSocketJoint(
            bodyA=engineCar.physicsBody,
            anchorA=scn.Vector3(max.x, min.y, 0),
            bodyB=wagon1.physicsBody,
            anchorB=scn.Vector3(wmin.x, wmin.y, 0),
        )
        scene.physicsWorld.addBehavior(joint)

        # Wagon1 & Wagon2
        joint = scn.PhysicsBallSocketJoint(
            bodyA=wagon1.physicsBody,
            anchorA=scn.Vector3(wmax.x + 0.1, wmin.y, 0),
            bodyB=wagon2.physicsBody,
            anchorB=scn.Vector3(wmin.x - 0.1, wmin.y, 0),
        )
        scene.physicsWorld.addBehavior(joint)
    ...
    def setupVehicle(self, scene):
        ...
        chassisNode = carScene.rootNode.childNodeWithName(
            "rccarBody", recursively=False
        )
        ...
        self.myReactor.idleDurationVariation = 0.0
        ...
        self.myReactor.particleVelocity = 40.0
        self.myReactor.particleVelocityVariation = 40.0
        self.myReactor.particleImage = ui.Image.named("resources/spark.png")
        self.myReactor.particleSize = 1.0
        self.myReactor.particleSizeVariation = 1.0
        self.myReactor.particleIntensity = 1.0
        self.myReactor.particleIntensityVariation = 0.4
        self.myReactor.stretchFactor = 0.0
        self.myReactor.particleColor = (0.87, 0.45, 0.0)
        ...
        vehicle = scn.PhysicsVehicle(
            chassisBody=chassisNode.physicsBody, wheels=[wheel0, wheel1, wheel2, wheel3]
        )
    ...
    def didSimulatePhysics(self, view, atTime):
        ...
        cameraPos = scn.Vector3(
            cameraPos.x * (1.0 - self.cameraDamping) + targetPos.x * self.cameraDamping,
        ...
        if not self.inCarView:
            ...
            frontPosition = self.scnView.pointOfView.presentationNode.convertPosition(
                scn.Vector3(0, 0, -30), toNode=scn.nil
            )
            self.mySpotLightNode.position = scn.Vector3(
                frontPosition.x, 80.0, frontPosition.z
            )
        ...
        self.overlayScene.mySpeedNeedle.zRotation = -(
            abs(self.myVehicle.speedInKilometersPerHour) * M_PI / self.maxSpeed
        )
    ...
    def changePointOfView(self):
        ...
        if index == len(self.pointOfViews):
            index = 0
