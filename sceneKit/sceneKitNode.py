...
class Node(Actionable, Animatable, BoundingVolume, CInst):
    ...
    def childNodeWithName(self, name, recursively=False):
        return sceneKit.Node.outof(
            self.ID.childNodeWithName_recursively_(name, recursively)
        )
    ...
    def hitTestWithSegment(self, fromPoint=None, toPoint=None, options=None):
        ...
        hitList = self.ID.hitTestWithSegmentFromPoint_toPoint_options_(
            fpv, tpv, hitTestOptions(options)
        )
        return tuple([sceneKit.HitTestResult.outof(aHit) for aHit in hitList])

    def rotateBy(self, worldRotation=None, worldTarget=None):
        self.ID.rotateBy_aroundTarget_(
            quaternionMake(worldRotation), vector3Make(worldTarget)
        )
    ...
    def lookAtUpLocalFront(self, worldTarget=None, worldUp=None, localFront=None):
        self.ID.lookAt_up_localFront_(
            vector3Make(worldTarget), vector3Make(worldUp), vector3Make(localFront)
        )
    ...
    localRight = Vector3(
        SCNNode.localRight().a, SCNNode.localRight().b, SCNNode.localRight().c
    )
    localUp = Vector3(SCNNode.localUp().a, SCNNode.localUp().b, SCNNode.localUp().c)
    localFront = Vector3(
        SCNNode.localFront().a, SCNNode.localFront().b, SCNNode.localFront().c
    )
    ...
    def convertPosition(self, position=None, fromNode=None, toNode=sceneKit.Nil):
        if fromNode is not None:
            return vector3Make(
                getCStructValuesAsList(
                    self.ID.convertPosition_fromNode_(
                        vector3Make(position), fromNode.ID
                    )
                )
            )
        else:
            return vector3Make(
                getCStructValuesAsList(
                    self.ID.convertPosition_toNode_(vector3Make(position), toNode.ID)
                )
            )
    ...
    def convertTransform(self, transform=None, fromNode=None, toNode=sceneKit.Nil):
        if fromNode is not None:
            return matrix4Make(
                getCStructValuesAsList(
                    self.ID.convertTransform_fromNode_(
                        matrix4Make(transform), fromNode.ID
                    )
                )
            )
        else:
            return matrix4Make(
                getCStructValuesAsList(
                    self.ID.convertTransform_toNode_(matrix4Make(transform), toNode.ID)
                )
            )
    ...
    def convertVector(self, vector=None, fromNode=None, toNode=sceneKit.Nil):
        if fromNode is not None:
            return vector3Make(
                getCStructValuesAsList(
                    self.ID.convertVector_fromNode_(vector3Make(vector), fromNode.ID)
                )
            )
        else:
            return vector3Make(
                getCStructValuesAsList(
                    self.ID.convertVector_toNode_(vector3Make(vector), toNode.ID)
                )
            )
...
class ChildNodesPassingTestBlock:
    def __init__(self, block):
        self.blockCode = ObjCBlock(
            self.blockInterface,
            restype=c_bool,
            argtypes=[c_void_p, c_void_p, POINTER(c_bool)],
        )
        self.pCode = block
